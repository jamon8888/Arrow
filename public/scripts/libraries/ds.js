...
(function() {
	...
	window.DataSift = {
		...
		connect: function(username, apikey, host) {
			...
			host = host || 'websocket.datasift.com';
		...
		jsonp: function() {
			...
			for (var hash in window.Datasift) {
				if (window.Datasift.hasOwnProperty(hash)) {
					...
					script.src = 'http://api.datasift.com/fstream.jsonp?hash=' + hash + '&count=200&callback=window.Datasift[\'' + hash + '\'].data' + (window.Datasift[hash].lastInteractionId !== undefined ? '&interaction_id=' + window.Datasift[hash].lastInteractionId : '') + '&random=' + Math.random() ;
					script.id = 'ds-' + hash;
		...
		broadcast: function(action, data, h) {
			...
			if(!h){
				for (var hash in window.Datasift) {
					if (window.Datasift.hasOwnProperty(hash)) {
						l = window.Datasift[hash].callbacks.length;
						for (var i = 0; i < l; i++) {
				...
				l = window.Datasift[h].callbacks.length;
				for (var i = 0; i < l; i++) {
